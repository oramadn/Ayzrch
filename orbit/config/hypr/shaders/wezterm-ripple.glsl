...
void main() {
    ...
    float speed = exp(-effect_time * 2.8) * smoothstep(2.0, 1.2, effect_time);
    float wave_phase = edge_distance * 34.0 - effect_time * 18.0;
    float front = exp(-pow((edge_distance - effect_time * 0.42) * 9.0, 2.0));
    float edge_hit = exp(-effect_time * 6.0) * exp(-edge_distance * 90.0);
    float wave = sin(wave_phase) * front + edge_hit;
    float envelope = exp(-edge_distance * 2.5) * speed;
    vec2 offset = direction * wave * envelope * 0.0065 * size_scale;

    float chroma = (front + edge_hit) * speed * 0.0035 * size_scale;
    ...
    float effect_strength = clamp((front + edge_hit) * speed, 0.0, 1.0);
    vec3 colour = vec3(red, green, blue);
    colour *= 1.0 + effect_strength * 0.05;
    alpha = min(1.0, alpha + effect_strength * 0.18);
